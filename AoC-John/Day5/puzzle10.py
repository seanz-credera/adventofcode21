...
class Line:
    ...
    def is_pos_diag(self):
        return float(self.p2.y - self.p1.y) / (self.p2.x - self.p1.x) < 0

    def is_neg_diag(self):
        return float(self.p2.y - self.p1.y) / (self.p2.x - self.p1.x) > 0
            ...
            if line.is_vertical():
                ...
                if p1x > p2x:
                    ...
                    for x in range(p1x, p2x + 1):
                        self.grid[x][p1y] += 1
            # Line is y=x+b
            elif line.is_pos_diag():
                if p1x > p2x:
                    for i in range(abs(p2x - p1x) + 1):
                        self.grid[p2x + i][p2y - i] += 1
                else:
                    for i in range(abs(p2x - p1x) + 1):
                        self.grid[p1x + i][p1y - i] += 1
            # Line is y=-x+b
            else:
                if p1x > p2x:
